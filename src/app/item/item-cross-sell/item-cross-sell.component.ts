    ...
    onRemove(itemCrossSell: ItemCrossSell) {
        ...
        if (confirmation) {
            this.itemService.deleteItemCrossSell(itemCrossSell.ItemCrossSellID).subscribe(
                () => {                
                    const itemlist = new ItemList(itemCrossSell.CrossSellItemID, itemCrossSell.CrossSellItemVendorSKU + ' - ' + itemCrossSell.CrossSellItemName + ' - ' + itemCrossSell.CrossSellTPIN, 0, null, null, null);
