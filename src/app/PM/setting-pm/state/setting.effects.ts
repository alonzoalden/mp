...
export class SettingEffects {
    ...
    @Effect()
    loadAllSubMemberVendors$: Observable<Action> = this.actions$.pipe(
        ofType(settingActions.SettingActionTypes.LoadAllSubMemberVendors),
        mergeMap(() =>
            this.settingService.getAllSubMemberVendors().pipe(
                map((members: MemberVendor[]) => (new settingActions.LoadMemberVendorsSuccess(members))),
                catchError(err => {
                    of(new settingActions.LoadMemberVendorsFail(err));
                    return EMPTY;
